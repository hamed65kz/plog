...
namespace plog
{
    ...
    public:
        ...
        static util::nstring format(const Record& record)
        {
            ...
            ss << std::setfill(PLOG_NSTR(' '))<<PLOG_NSTR("[^") << std::setw(5) << std::left << severityToString(record.getSeverity()) << PLOG_NSTR("&]");
            //ss << PLOG_NSTR("[") << record.getTid() << PLOG_NSTR("] ");
            ss << PLOG_NSTR("[") << record.getFunc() << PLOG_NSTR("@") << record.getLine() << PLOG_NSTR("] ");
            ss <<PLOG_NSTR("^")<< record.getMessage() <<PLOG_NSTR("&")<< PLOG_NSTR("\n");
